  ...
  getWeatherStatus(lat: any, long: any) {
    ...
    this.WeatherService.GetWeatherStatus(lat, long).subscribe((response) => {
      ...
      if (this.Weather) {
        ...
        if (this.Weather.weather[0].main === "Rain"|| this.Weather.weather[0].main==='Mist'||this.Weather.weather[0].main==='Thunderstorm'|| this.Weather.weather[0].main === 'Drizzle') {
          this.viewRain = true;
          this.viewSunny = false;
          this.viewCloud = false;
        }
        if (this.Weather.weather[0].main === "Clouds"||this.Weather.weather[0].main==="Haze") {
          this.viewCloud = true;
          this.viewSunny = false;
          this.viewRain = false;
        }
